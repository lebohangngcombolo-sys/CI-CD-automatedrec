...
class GoogleCalendarService:
    ...
    def authenticate(self):
        try:
            token_path = current_app.config.get('GOOGLE_CALENDAR_TOKEN_PATH')
            credentials_path = current_app.config.get('GOOGLE_CALENDAR_CREDENTIALS_PATH')

            if not token_path or not credentials_path:
                current_app.logger.error("Google Calendar paths not set in environment")
                return False
